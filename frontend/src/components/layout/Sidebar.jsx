...
import {
  ...
  Plus, Upload, Database, Layers
} from 'lucide-react';
import { useAuth } from '../../context/AuthContext';
import { formatBytes } from '../../utils/formatters';

export default function Sidebar({ currentPath, onNavigate, onUploadClick, onNewFolderClick, storageStats }) {
  ...
  return (
    <aside className="flex h-[calc(100vh-4rem)] w-64 flex-col justify-between border-r border-slate-200/90 bg-white p-4 shadow-xs overflow-y-auto">
      <div className="space-y-4 text-left">
        {/* Action Buttons: Upload File & New Folder */}
        <div className="space-y-2">
          <button
            onClick={onUploadClick}
            className="flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-blue-700 via-indigo-600 to-blue-600 px-4 py-2.5 text-xs font-bold text-white shadow-md shadow-indigo-500/25 transition hover:from-blue-800 hover:to-indigo-700 active:scale-[0.98]"
          >
          ...
          <button
            ...
            className="flex w-full items-center justify-center gap-2 rounded-xl border border-slate-200 bg-slate-50/80 px-4 py-2 text-xs font-semibold text-slate-700 shadow-xs transition hover:bg-slate-100 hover:border-slate-300"
          >
