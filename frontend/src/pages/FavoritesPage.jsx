...
export default function FavoritesPage() {
  ...
  const loadStarredFiles = async () => {
    setLoading(true);
    try {
      const data = await api.get('/files', { scope: 'favorites' });
      setFiles(data.files || []);
    } catch (err) {
      console.warn('Failed to load starred files:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-5 text-left">
      <div className="flex items-center justify-between border-b border-slate-200 pb-3 dark:border-slate-800">
        <div>
          ...
          <p className="text-xs text-slate-400">
            Quick access to your prioritized and starred files.
          </p>
        </div>
      ...
      {loading ? (
        ...
        <FileTable
          files={files.map(f => ({ ...f, is_starred: 1 }))}
          onPreview={(f) => setSelectedPreviewFile(f)}
          onDownload={handleDownload}
          onShare={(f) => setSelectedShareFile(f)}
          onToggleStar={handleToggleStar}
